...
const ActivityBar: React.FC<ActivityBarProps> = ({ activeView, setActiveView }) => {
    ...
    return (
        <div className="bg-[var(--background-secondary)]/70 backdrop-blur-md h-full w-12 flex flex-col items-center py-4 space-y-2 border border-[var(--border-color)] rounded-lg shadow-lg">
            {views.map(view => (
                <button
                    key={view.id}
                    title={view.title}
